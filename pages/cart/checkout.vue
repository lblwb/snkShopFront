...
function isCheckoutAllowed() {
  // Проверка всех условий для отображения кнопки checkout
  if (!InputConfirmTerm.checked) {
    return false;
  }

  if (!InputSelPayment.selected) {
    return false;
  }

  if (InputCountryDelSel.selected !== null && InputCountryDelSel.selected.id && InputCountryDelSel.selected.id == 'OTH') {
    return InputAddress.value !== undefined && InputAddress.value === null || InputAddress.value.length > 3;
  } else {
    const InputPrcCntryDelSelRaw = toRaw(InputPrcCntryDelSel).value;
    return (InputPrcCntryDelSelRaw.selected !== undefined && InputPrcCntryDelSelRaw.selected !== null && InputPrcCntryDelSelRaw.selected.zip_code !== undefined && InputPrcCntryDelSelRaw.selected.zip_code !== null);
  }
